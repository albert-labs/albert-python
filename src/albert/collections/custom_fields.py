...
class CustomFieldCollection(BaseCollection):
    ...
    def update(self, *, custom_field: CustomField) -> CustomField:
        ...
        payload = self._generate_custom_field_patch_payload(
    ...
    def _generate_custom_field_patch_payload(
        ...
        for attribute in self._updatable_attributes:
            ...
            if alias == "entityCategory":
                data.extend(
                    self._generate_entity_category_patches(
            ...
            if alias == "customEntityCategory":
                data.extend(
                    self._generate_custom_entity_category_patches(
                        attribute=alias, old_value=old_value, new_value=new_value
                    )
                )
                continue
    ...
    def _generate_custom_entity_category_patches(
        *, attribute: str, old_value: list | None, new_value: list | None
        ...
        if old_value is not None and new_value is None:
            return [
                PatchDatum(
                    attribute=attribute, operation=PatchOperation.DELETE, old_value=old_value
                )
            ]
