...
class DataTemplateCollection(BaseCollection):
    ...
    def add_parameters(
        ...
        saved_enums = strip_new_parameter_enums(parameters)
        ...
        push_new_parameter_enums(
            ...
            parameters_base_url=f"{self.base_path}/{data_template_id}/parameters",
    ...
    def update(self, *, data_template: DataTemplate) -> DataTemplate:
        ...
        if len(new_parameters) > 0:
            saved_enums = strip_new_parameter_enums(new_parameters)
            response = self.session.put(
            ...
            push_new_parameter_enums(
                session=self.session,
                parameters_base_url=f"{self.base_path}/{existing.id}/parameters",
                parameters=returned_parameters,
            )
        enum_sequences = {}
