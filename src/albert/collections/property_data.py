...
class PropertyDataCollection(BaseCollection):
    ...
    def get_all_task_properties(
        self, *, task_id: TaskId, data_exists_only: bool = False
        ...
        data_exists_only : bool, optional
            When True, only return combinations whose ``dataExist`` flag is true. Defaults to False.

        Returns
        -------
        list[TaskPropertyData]
            Task property data for each block/inventory/lot combination. When
            ``data_exists_only`` is True, combinations without recorded data are omitted.
        """
        all_info = []
        task_data_info = self.check_for_task_data(task_id=task_id)
        for combo_info in task_data_info:
            if data_exists_only and not combo_info.data_exists:
                continue
            all_info.append(
