...
class RequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.server.jwt = None
        self.server.refresh_token = None
        self.server.error = None

        parsed = urlparse(self.path)
        query_str = parse_qs(parsed.query)
        self.server.jwt = query_str.get("jwt", [None])[0]
        self.server.refresh_token = (
            query_str.get("token", [None])[0] or query_str.get("refreshtoken", [None])[0]
        )

        status = (
            "successful"
            if self.server.jwt or self.server.refresh_token
            else "failed (no token found)"
        )
...
def start_local_http_server(
    ...
    port = _find_open_port(minimum_port, maximum_port)
    if port is None:
        raise RuntimeError(
