...
Start at [`tests/TESTING.md`](../TESTING.md) to decide whether a test is unit or
integration. This guide covers `tests/integration/` only.
...
- **File uses only `client` / function-scoped fixtures**: leave it unmarked so
...
## Rule 4: seeding conventions (`tests/integration/conftest.py` / `seeding.py`)

- Seed data generators live in `tests/integration/seeding.py`; every generated name embeds `seed_prefix`.
...
- No HTTP fakes or mocks here; a test that fakes the transport belongs in `tests/unit/`.
...
```bash
uv run pytest tests/integration -n 4                          # parallel, same as CI
uv run pytest tests/integration/collections/test_tags.py -v   # single file, sequential
```
